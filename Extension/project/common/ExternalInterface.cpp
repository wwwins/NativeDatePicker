...
static value extension_init_datePicker() {
	bool result = initDatePicker();
	return alloc_bool(result);
}
DEFINE_PRIM (extension_init_datePicker, 0);

void extension_show_datePicker () {
	showDatePicker();
}
DEFINE_PRIM (extension_show_datePicker, 0);


void extension_remove_datePicker () {
	removeDatePicker();
}
